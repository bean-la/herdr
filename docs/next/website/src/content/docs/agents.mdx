...
Remote presence rows use the same configured multi-line layout as local rows. In addition to the usual `machine`, `workspace`, `tab`, `agent`, and `state_text` values, the default layout can show `$kind` (`Pi` for project-user sessions and `HRM` for the `herm` project) and `$last_seen` when the presence feed supplies a valid timestamp. `$context` is populated only when the feed contains an explicit context-usage value. Mailbox state and last-message time are not part of the presence wire response, so those tokens remain absent rather than displaying guessed or stale values. Remote rows are read-only and are never click targets.

The agent-panel `remotes`/`local` selector remains available for both direct and multi-machine clients: it filters rows from each endpoint snapshot. Multi-machine rendering keeps local pane ordering and appends endpoint remote-presence rows, so the selector is effective on laptop clients as well as on VPS clients. It is intentionally not removed merely because a current snapshot has no remote rows; presence can arrive on a later refresh.
