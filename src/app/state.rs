    ...
    pub fn items(&self) -> Vec<&'static str> {
        match self.kind {
            ...
            ContextMenuKind::Pane {
                ...
                if has_manual_label { items.push("Clear pane name"); }
                if source_pane_id.is_some() { items.push("Swap with focused pane"); }
                items.extend(["Split right", "Split down", "Zoom", "Pin pane", "Unpin pane"]);
                items.push(if right_click_passthrough { "Use Herdr right-click menu" } else { "Send right-clicks to pane" });
